...
/**
 * Shows the author the reviews on his paper only if the PC has made a decision.
 * @author James
 * Adapted from ShowReviews class
 */
public class ShowAuthorReview {
	 ...
	 */
	public ShowAuthorReview(final User theUser, final Paper thePaper, final List<Review> theReviews) {
		myFrame = new JFrame();
		myUser = theUser;
		myPaper = thePaper;
	...
	private void createWindow() {
		...
		//if the pc has made his final decision.
		if (myUser.getIntPaperStatus(myPaper) > 2) {
			...
			for (int j = 0; j < myReviews.size(); j++) {
				...
				theInfo.append("Currently Awaiting Judgement.");
				theInfo.append("\n\n");
				theInfo.append("----------------------------------------------------------------------------------");
				theInfo.append("\n\n");
